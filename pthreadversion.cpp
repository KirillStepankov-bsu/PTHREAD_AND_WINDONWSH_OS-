...
void *Call(void *args) {
    ...
    multiplyBlocks(castedArgs->firstMatrix, castedArgs->secondMatrix, castedArgs->result, castedArgs->I, castedArgs->J,
                   castedArgs->n, castedArgs->r);
    pthread_exit(0);
}
...
void multiplyMatrixThread(const std::vector<int> &a, const std::vector<int> &b, std::vector<int> &c, int n, int r) {
    ...
    for (int I = 0; I < n; I += r) {
        for (int J = 0; J < n; J += r) {
            threads.emplace_back(pthread_create(&tid, &attr, Call, new Args(a, b, c, I, J, n, r)));
        }
    }
